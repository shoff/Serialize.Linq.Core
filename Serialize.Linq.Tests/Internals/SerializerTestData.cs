...
namespace Serialize.Linq.Tests.Internals
{
    ...
    {
        private static readonly int[] arrayOfIds = { 6, 7, 8, 9, 10 };
        private static readonly List<int> listOfIds = new List<int> { 6, 7, 8, 9, 10 };
        ...
        {
            (Expression<Func<Bar, bool>>)(p => arrayOfIds.Contains(p.Id)),
            (Expression<Func<Bar, bool>>)(p => listOfIds.Contains(p.Id))
        };
